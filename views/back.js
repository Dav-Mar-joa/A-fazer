      ...
      .purchase-container
        p.titre A dire
          form(action="/phrases" method="post")
            .PhraseQui
              label(for="sayTo") Qui :
              input(type="text" id="sayTo" name="sayTo" required class="input-qui")
            label(for="say") Quoi :
            input(type="text" id="say" name="say" required)
            button(type="submit" class="button-submit") Ajouter  
        
        each phrase in phrases
          .purchase-item(data-phrase-id=phrase._id)
            .PhraseQui
              h3.phraseQuiItem #{phrase.qui}
              h3.phrase #{phrase.name} 
            button(type="button" onclick="deletePhrase(this)" class="button-delete") Supprimer                         
